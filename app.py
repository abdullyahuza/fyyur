...
def index():
  ...
  return render_template('pages/home.html', artists = new_listed_artists, venues = new_listed_venues)
...
def create_venue_submission():
  ...
  if request.method == "POST":
    try:
      ...
      v_seeking_talent = request.form.get('seeking_talent') == 'y' if True else False
      v_seeking_description = request.form.get('seeking_description', default='')
    ...
    if error:
      ...
      if error != True:
        flash({'type': 'success', 'msg':'Venue ' + venue_success + ' was successfully listed!'})
      else:
        flash({'type':'error','msg':'An error occurred. Venue ' + v_name + ' could not be listed.'})
      return redirect(url_for('index'))
  else:
...
def delete_venue(venue_id):
  ...
  if error:
    ...
    flash({'type': 'success', 'msg':'Venue ' + venue_name + ' deleted successfully!'})
    return redirect(url_for('index'))
    
  flash({'type': 'error', 'msg':'Venue ' + venue_name + ' could not be deleted!'})
...
def show_artist(artist_id):
  ...
  if ArtistData.looking_for_venues == True:
...
def edit_artist_submission(artist_id):
  ...
  if artist.id == artist_id :
    try:
      ...
      artist.looking_for_venues = request.form.get('seeking_venue') == 'y' if True else False
    ...
    if error:
      ...
      if error != True:
        flash({'type': 'success', 'msg':'Artist ' + artist_name + ' updated successfully!'})
      else:
        flash({'type':'error','msg':'An error occurred. Artist ' + artist_name + ' could not be updated.'})
...
def edit_venue_submission(venue_id):
  ...
  if venue.id == venue_id :
    try:
      ...
      venue.looking_for_talent = request.form.get('seeking_talent') == 'y' if True else False
    ...
    if error:
      ...
      if error != True:
        flash({'type': 'success', 'msg':'Venue ' + venue_name + ' updated successfully!'})
      else:
        flash({'type':'error','msg':'An error occurred. Venue ' + venue_name + ' could not be updated.'})
...
def create_artist_submission():
  ...
  if request.method == 'POST':
    try:
      ...
      a_seeking_venue = request.form.get('seeking_venue') == 'y' if True else False
      a_seeking_description = request.form.get('seeking_description', default='')
    ...
    if error:
      ...
      if error != True:
        flash({'type': 'success', 'msg':'Artist ' + artist_success + ' was successfully listed!'})
      else:
        flash({'type':'error','msg':'An error occurred. Artist ' + a_name + ' could not be listed.'})
...
def shows():
  ...
  for s in shows:
    ...
    show['venue_name'] = db.session.query(Venue.name).filter(Venue.id==s.venueID).first()[0]
    show['artist_id'] = s.artistID
    show['artist_name'] = db.session.query(Artist.name).filter(Artist.id==s.artistID).first()[0]
    show['artist_image_link'] = db.session.query(Artist.image_link).filter(Artist.id==s.artistID).first()[0]
...
def create_show_submission():
  ...
  if request.method == 'POST':
    try:
      ...
      db.session.close()
    if error:
      flash({'type':'error','msg':'An error occurred. Show could not be listed.'})
    else:    
      if error != True:
...
def add_availabilty(artist_id):
  if request.method == 'GET' :
    ...
    if artist:
      ...
      return render_template('forms/set_availability.html', form=form, name=artistName, artist_id=artist_id)
    ...
    try:
      ...
      db.session.close()
    if error:
      flash({'type':'error','msg':'An error occurred. Availability could not be added.'})
    else:    
      if error != True:
        flash({'type': 'success', 'msg':'Availability added successfully!'})
      else:
        flash({'type':'error','msg':'An error occurred. Availability could not be added.'})
